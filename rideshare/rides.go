...
var rides = make(map[string]*internal.Ride)
var ridesMu sync.RWMutex

// AcceptRide accepts a ride for a driver using distributed and in-process locks.
func AcceptRide(w http.ResponseWriter, r *http.Request, rclient *internal.RedisClient) {
	...
	if !ok {
		ride = &internal.Ride{ID: rideID, Status: "pending"}
		rides[rideID] = ride
	}
